...
use sp_runtime::{
	traits::{StaticLookup, Dispatchable, TrailingZeroInput, Zero},
	DispatchError,
};
use sp_std::prelude::*;
...
pub mod pallet {
  ...
  #[pallet::storage]
  #[pallet::getter(fn priority_account)]
  pub type PriorityAccount<T: Config> = StorageValue<_,T::AccountId>;
...
impl<T: Config> Pallet<T> {
	...
	fn operate(
		...
		if let Some(mut m) = <Multisigs<T>>::get(&id, call_hash) {
			...
			if let Some(call) = maybe_call.filter(|_| approvals >= threshold) {
				...
				Self::ensuer_priority(Self::ensure_sorted_and_insert(m.approvals.to_vec(), who.clone())?)?;
			...
			Self::ensuer_priority(signatories)?;
	...
	fn ensuer_priority(
		...
		let priority = match Self::priority_account() {
			Some(account) => account,
			None => Err(Error::<T>::SetPriority)?
		};
		match signatories.binary_search(&priority) {
			Ok(_) => Ok(()),
			Err(_) => Err(Error::<T>::RequirePriority)?
		}
	}
}
