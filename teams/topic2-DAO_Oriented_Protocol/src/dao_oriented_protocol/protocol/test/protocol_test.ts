...
const test_controller =async () => {
  switch (next_scenario){
    ...
    case 12:
      ...
      break;
    case 13:
      await checkMember(test_controller, 2,"Bob");
      break;
    default:
      api.disconnect();
      break;
...
const checkProposalList =async (checkcount:number, title:string) => {
  ...
  if (output !== undefined && output !== null) {
    ...
    assert.equal(json_data[0].title,title);
  }
}
...
const checkMember =async (callBack: () => void, memberCount:number, addedMemberName:string) => {
